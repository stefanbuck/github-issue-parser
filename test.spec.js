...
it("empty issue", () => {
  const expectedOutput = require("./fixtures/empty-issue/expected.json");
  const expectedOutputJson = JSON.stringify(expectedOutput, null, 2);
  ...
  // mock event payload
  const eventPayload = require("./fixtures/empty-issue/issue");
  ...
  const fs = {
    readFileSync(path, encoding) {
      expect(path).toBe("<template-path>");
      expect(encoding).toBe("utf8");
      return readFileSync("fixtures/empty-issue/form.yml", "utf-8");
    },
    writeFileSync(path, content) {
      expect(path).toBe("<home path>/issue-parser-result.json");
      expect(content).toBe(expectedOutputJson);
